...
app.post("/upload", upload.single("images"), (req: any, res: Response) => {
  if (req.body.secret === process.env.RAMCHO_SECRET) {
    ...
    file.save((err, file) => {
      if (err)
        return res.json({
          error: "The file is not saved, something is wrong.",
        });
      res.json({
        thumbnail_url: `${process.env.API_URI}/cdn/${filename}.${
...
app.get("/i/:id", async (req, res) => {
  try {
    ...
    res.json({
      filename: file._id,
      type: file.type,
      url: `${process.env.API_URI}/cdn/${file._id}.${file.type.split("/")[1]}`,
    });
  } catch (err) {
    console.log(err);
...
app.get("/cdn/:id", async (req: any, res: any) => {
  try {
    ...
    if (!fileId[1].match(/^(jpe?g|png|gif|mp4)$/))
      return res.status(404).json({ error: "File not found", status: 404 });
